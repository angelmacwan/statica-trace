...
*Build order: tooling setup (M0) → schema + schema tests (M1) → backend + backend tests (M2) → SDK + SDK tests (M3) → frontend + frontend tests (M4) → polish (M5) → deploy (M5.2) → system tests pass (M7) → validate with users (M6) → v2 (M8).*
